...
exports.ogActualite = exports.recordVisit = exports.sendDirectMessage = exports.sendContactForm = exports.onRendezVousConfirme = exports.onConferencePublished = exports.onSubscriberCreated = exports.sendNewsletter = exports.processCheckout = void 0;
...
const icsDate = (d) => `${d.getUTCFullYear()}${deuxChiffres(d.getUTCMonth() + 1)}${deuxChiffres(d.getUTCDate())}T${deuxChiffres(d.getUTCHours())}${deuxChiffres(d.getUTCMinutes())}00Z`;
function icsRendezVousConfirme(id, debut, fin, salle) {
...
exports.onRendezVousConfirme = (0, firestore_1.onDocumentWritten)({
    ...
    const before = event.data?.before?.data();
    const after = event.data?.after?.data();
    if (!after?.statut || after.statut !== 'confirme' || before?.statut === 'confirme')
        return;
    if (!after.courriel || !after.debut || !after.fin || !after.salle)
        return;
    const user = emailUser.value();
    const pass = emailPass.value();
    if (!user || !pass) {
...
<body style="font-family:Arial,sans-serif;background:#f5f5f5;margin:0;padding:20px;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08);">
    ...
    try {
        ...
        await logEmail({ type: 'rendezvous_confirme', to: after.courriel, subject, html, success: false, errorMessage: err.message, meta: { rdvId: event.params.rdvId } });
...
exports.sendContactForm = (0, https_1.onCall)({
    ...
    const configuredDest = (contentSnap.data() ?? {})['contact_form_destination'] ?? '';
...
let coquille = null;
async function pageDeBase() {
    if (coquille && Date.now() - coquille.at < 10 * 60 * 1000)
        return coquille.html;
    try {
...
const echapper = (v) => v.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
const resumeTexte = (corps, max = 200) => {
    const plat = (corps || '').replace(/\s+/g, ' ').trim();
    return plat.length <= max ? plat : `${plat.slice(0, max - 1).trimEnd()}…`;
};
function balises(titre, description, image, url) {
...
function injecter(html, blocDeBalises) {
    const nettoye = html
...
exports.ogActualite = (0, https_1.onRequest)({ region: 'northamerica-northeast1', memory: '256MiB', maxInstances: 3, cors: false }, async (req, res) => {
