...
const AdminBlog = ({ posts, setPosts, mediaLibrary }: { posts: BlogPost[], setPosts: (posts: BlogPost[]) => void, mediaLibrary: string[] }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="bg-midnight/60 backdrop-blur-md rounded-xl border border-white/10 overflow-hidden">{posts.map((post, index) => (<div key={post.id} className={`p-6 flex flex-col md:flex-row gap-6 items-start md:items-center ${index !== posts.length - 1 ? 'border-b border-white/5' : ''}`}><div className="w-24 h-24 rounded-lg overflow-hidden bg-slate-800 flex-shrink-0"><img src={post.image} alt={post.title} className="w-full h-full object-cover" /></div><div className="flex-1"><div className="flex items-center gap-3 mb-1"><span className="text-gold text-xs font-bold uppercase tracking-wider">{post.date}</span><span className="w-1 h-1 rounded-full bg-slate-600"></span><span className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${post.isPublished ? 'bg-green-500/20 text-green-400' : 'bg-slate-600/50 text-slate-400'}`}>{post.isPublished ? 'Publié' : 'Brouillon'}</span></div><h3 className="text-xl font-bold text-white mb-2">{post.title}</h3><p className="text-slate-400 text-sm line-clamp-2">{post.excerpt}</p></div><div className="flex gap-2"><button onClick={() => openEditor(post)} className="p-2 bg-white/5 rounded-lg text-white hover:bg-gold hover:text-midnight transition-colors"><Edit3 size={18} /></button><button onClick={() => handleDeletePost(post.id)} className="p-2 bg-white/5 rounded-lg text-white hover:bg-red-500 hover:text-white transition-colors"><Trash2 size={18} /></button></div></div>))}
