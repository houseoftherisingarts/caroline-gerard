...
const TabButton = ({ id, label, active, badge, onClick }: { id: Tab; label: string; active: boolean; badge?: number; onClick: (t: Tab) => void }) => (
  <button
    onClick={() => onClick(id)}
...
const Bubble = ({ msg, isOwnMessage }: { msg: CommunityMessage; isOwnMessage: boolean }) => (
  <div className={`flex ${isOwnMessage ? 'justify-end' : 'justify-start'} mb-3`}>
    <div className={`max-w-[75%] px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
      ...
      {!isOwnMessage && <p className="text-xs font-bold text-gold mb-1">Caroline Gérard</p>}
      {msg.body}
      <p className={`text-[10px] mt-1 ${isOwnMessage ? 'text-midnight/60 text-right' : 'text-slate-500'}`}>
        ...
        {isOwnMessage && !msg.read && <span className="ml-1">· En attente</span>}
        {isOwnMessage && msg.read && <span className="ml-1">· Lu ✓</span>}
      </p>
    </div>
  </div>
);
...
const CommunautePage = ({ posts, events, conferences }: CommunautePageProps) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    catch (e: unknown) { setAuthError((e as Error).message ?? 'Erreur de connexion.'); }
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (code.includes('user-not-found') || code.includes('wrong-password') || code.includes('invalid-credential'))
        ...
        setAuthError('Ce courriel est déjà utilisé. Essayez de vous connecter.');
      else if (code.includes('weak-password'))
        setAuthError('Le mot de passe doit contenir au moins 6 caractères.');
      else setAuthError((e as Error).message ?? 'Erreur de connexion.');
    } finally { setAuthBusy(false); }
  };
  ...
  if (!user) return (
    <div className="min-h-screen pt-32 pb-16 px-6 flex flex-col items-center justify-center">
      <Helmet><title>Caroline Gérard | Communauté des Rêveurs</title></Helmet>
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gold/10 border border-gold/20 text-gold mb-4">
            ...
            <span className="text-xs font-bold uppercase tracking-widest">Communauté des Rêveurs</span>
          </div>
          <h1 className="font-serif text-4xl text-white mb-2">Votre espace membre</h1>
          <p className="text-slate-400">Actualités, commandes, événements et messagerie.</p>
        </div>
        <div className="bg-midnight/60 backdrop-blur-md border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            <div className="relative">
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Courriel" required
            ...
            <button type="submit" disabled={authBusy}
              ...
              {authBusy ? 'Connexion...' : isRegister ? 'Créer mon compte' : 'Se connecter'}
            </button>
          </form>
          <button onClick={() => { setIsRegister(!isRegister); setAuthError(''); }}
            ...
            {isRegister ? 'Déjà un compte? Se connecter' : "Pas de compte? S'inscrire gratuitement"}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="min-h-screen pt-32 pb-16 px-6 md:px-12">
      ...
      <div className="max-w-5xl mx-auto mb-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">Communauté des Rêveurs</p>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex gap-2 flex-wrap mb-8 p-2 bg-midnight/60 rounded-xl border border-white/10 w-fit">
          <TabButton id="actualites" label="Actualités" active={tab === 'actualites'} onClick={setTab} />
          <TabButton id="commandes" label="Mes commandes" active={tab === 'commandes'} onClick={setTab} />
          <TabButton id="evenements" label="Événements" active={tab === 'evenements'} onClick={setTab} />
          <TabButton id="messages" label="Messages" active={tab === 'messages'} badge={unreadFromCaroline} onClick={setTab} />
        </div>
        ...
        {tab === 'actualites' && (
          <div className="space-y-4">
            {publishedConfs.length === 0 && publishedPosts.length === 0
              ? <p className="text-slate-500 text-center py-16">Aucune actualité pour le moment.</p>
              : <>
                  {publishedConfs.slice(0, 3).map(c => (
                    <div key={c.id} className="bg-midnight/60 border border-white/10 rounded-xl p-6 flex gap-4 items-start hover:border-gold/30 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-xs text-gold font-bold uppercase tracking-wider">Conférence</span>
                  ...
                  {publishedPosts.slice(0, 6).map(post => (
                    <div key={post.id} className="bg-midnight/60 border border-white/10 rounded-xl p-6 flex gap-4 items-start hover:border-gold/30 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-xs text-blue-400 font-bold uppercase tracking-wider">Article de blog</span>
        ...
        {tab === 'commandes' && (
          <div>
            {ordersLoading
              ? <p className="text-slate-500 text-center py-16 animate-pulse">Chargement de vos commandes...</p>
              : orders.length === 0
                ? <div className="text-center py-16"><ShoppingBag className="w-12 h-12 text-slate-700 mx-auto mb-4" /><p className="text-slate-500">Aucune commande trouvée pour ce compte.</p></div>
                : <div className="space-y-4">
                    {orders.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(order => (
                      <div key={order.id} className="bg-midnight/60 border border-white/10 rounded-xl p-6">
                        ...
                        {order.status === 'Envoyé' && (
                          <div className="mt-3 flex items-center gap-2 text-green-400 text-sm"><CheckCircle className="w-4 h-4" /> Votre livre est en route!</div>
                        )}
                      </div>
                    ))}
                  </div>
        ...
        {tab === 'evenements' && (
          <div className="space-y-4">
            {publishedEvents.length === 0
              ? <div className="text-center py-16"><Calendar className="w-12 h-12 text-slate-700 mx-auto mb-4" /><p className="text-slate-500">Aucun événement à venir.</p></div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                      ...
                      <div className="flex flex-col gap-2 shrink-0">
                        <a href={googleCalendarLink(event)} target="_blank" rel="noopener noreferrer"
                          ...
                          <ExternalLink className="w-4 h-4" /> Google Agenda
                        </a>
                        <button onClick={() => downloadICS(event)}
                          ...
                          <Download className="w-4 h-4" /> Télécharger .ics
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        {tab === 'messages' && (
          <div className="bg-midnight/60 border border-white/10 rounded-2xl overflow-hidden flex flex-col" style={{ height: '65vh' }}>
            ...
            <div className="px-6 py-4 border-b border-white/10 flex items-center gap-3 shrink-0">
              ...
              <div>
                <p className="text-white font-bold">Caroline Gérard</p>
                <p className="text-slate-500 text-xs">Messagerie privée</p>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6">
              {messages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-slate-600 text-xs mt-1">Envoyez un message à Caroline ci-dessous.</p>
                </div>
              )}
              {messages.map(msg => (
